...
use crate::memory_tools::{
    ...
    SessionRegistry, SessionSnapshot, SessionStats,
...
fn session_ids_with_nodes(repo: &Repository) -> std::collections::HashSet<String> {
    ...
    repo.list_paths(
        "main",
        SESSION_NODE_ROOT,
        Some(SESSION_DISCOVERY_MAX_DEPTH),
    )
        .unwrap_or_default()
        .iter()
...
async fn run_ingest_for_source(
    ...
    // The ingest subprocess resolves its own extraction provider + key (env,
    // `~/.ctxone/keys/<provider>`, or `~/.ctxone/extraction.toml`) and inherits
    // the hub's env, so the sweep runs memory extraction when configured and
    // otherwise just captures turns + tokens. No key handling needed here.
