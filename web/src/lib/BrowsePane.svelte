<script lang="ts">
	...
	// `target` lets a parent page (Search/Recall/Why) drive the selection: set
	// it to a stored path (or a section path whose leaf lives under it) and the
	// pane selects it and expands its ancestors. Bindable so the parent can
	// clear it. `onselect` fires whenever the pane's own selection changes.
	let {
	...
	// Resolve + select whenever the parent hands us a `target`. Recall/why hand
	// out *section* paths for pinned memories, while the path list holds the
	// stored leaves (…/body, …/title) — fall back to a child leaf when there's
	// no exact match. Runs after paths load so the resolution can see them.
	$effect(() => {
		const wanted = target;
		if (!wanted) return;
		if (paths.length === 0) return;
		const resolved = paths.includes(wanted)
			? wanted
			: (paths.find((p) => p === `${wanted}/body`) ??
				paths.find((p) => p.startsWith(`${wanted}/`)));
		if (!resolved) return;
		const next = new Set(expanded);
		...
		expanded = next;
		void selectPath(resolved);
	});

	export async function selectPath(path: string) {
		selectedPath = path;
		forgetMessage = null;
		onselect?.(path);
		try {
